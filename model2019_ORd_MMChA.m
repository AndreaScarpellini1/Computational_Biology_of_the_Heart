function output=model2019_ORd_MMChA(t,X,gAmps,settings,isq_act,corr_act,msIsqIni,msIsqTotal,num_beat,flag_ode)    
...
Jrelp=X(40);
CaMKt=X(41);
ko=X(42); %concentración extracel potasio, dinámica--------------------
...
thf=1.0/(3.6860e-6*exp(-(v+3.8875)/7.8579)+16*exp((v-0.4963)/9.1843));       %Dutta 
...
tj=4.8590+1.0/(0.8628*exp(-(v+116.7258)/7.6005)+1.1096*exp((v+6.2719)/9.0358));     %Dutta 
dj=(jss-j)/tj;
...
INa=f_pHi_INa*f_LPC_INa*GNa*gAmps(7)*(v-ENa)*m^3.0*((1.0-fINap)*h*j+fINap*hp*jp);
...
GNaL=0.0171;                   %Dutta (CinC2016)
if celltype==1
    GNaL=GNaL*0.6;
end
fINaLp=(1.0/(1.0+KmCaMK/CaMKa));
INaL=f_LPC_INaL*GNaL*gAmps(8)*(v-ENa)*mL*((1.0-fINaLp)*hL+fINaLp*hLp);
...
Ito=Gto*gAmps(2)*(v-EK)*((1.0-fItop)*a*i+fItop*ap*ip);
...
ICaL=gAmps(1)*f_alt_CaL*f_pHi_ICaL*f_pHo_ICaL*((1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca));
% ICaL=(1.0-fICaLp)*PCa*PhiCaL*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCap*PhiCaL*d*(fp*(1.0-nca)+jca*fcap*nca);
% 
ICaNa=gAmps(1)*f_alt_CaL*f_pHi_ICaL*f_pHo_ICaL*((1.0-fICaLp)*PCaNa*PhiCaNa*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCaNap*PhiCaNa*d*(fp*(1.0-nca)+jca*fcap*nca));
% ICaNa=(1.0-fICaLp)*PCaNa*PhiCaNa*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCaNap*PhiCaNa*d*(fp*(1.0-nca)+jca*fcap*nca);
% 
ICaK=gAmps(1)*f_alt_CaL*f_pHi_ICaL*f_pHo_ICaL*((1.0-fICaLp)*PCaK*PhiCaK*d*(f*(1.0-nca)+jca*fca*nca)+fICaLp*PCaKp*PhiCaK*d*(fp*(1.0-nca)+jca*fcap*nca));
...
% GKr=0.046;
GKr=0.0515;          %Dutta (CinC2016)
if celltype==1
    GKr=GKr*1.3;
elseif celltype==2
    GKr=GKr*0.8;
end
IKr=GKr*gAmps(5)*sqrt(ko/5.4)*xr*rkr*(v-EK);
...
% GKs=0.0034;
GKs=0.0056;     %Dutta (CinC2016)
if celltype==1
    GKs=GKs*1.4;
end
IKs=GKs*gAmps(6)*KsCa*xs1*xs2*(v-EKs);
...
% GK1=0.1908;
GK1=0.2698;    %Dutta (CinC2016)
if celltype==1
    GK1=GK1*1.2;
elseif celltype==2
    GK1=GK1*1.3;
end
IK1=GK1*gAmps(4)*sqrt(ko)*rk1*xk1*(v-EK);
...
INaCa_i=f_pHi_NCX*0.8*Gncx*gAmps(3)*allo*(zna*JncxNa+zca*JncxCa);
...
JncxNa=3.0*(E4*k7-E1*k8)+E3*k4pp-E2*k3pp;
JncxCa=E2*k2-E1*k1;
INaCa_ss=f_pHi_NCX*0.2*Gncx*gAmps(3)*allo*(zna*JncxNa+zca*JncxCa);
...
INaK=gAmps(9)*FPatri*f_ATP_INaK*f_pHi_INaK*Pnak*(zna*JnakNa+zk*JnakK);
...
%IKATP = G_KATP*(ko/5.4)^0.24*f_Mg*f_Na*
% 
% f_ATP_KATP*(v-EK);mariana
...
tau_relp=max(btp/(1.0+0.0123/cajsr),0.005);
...
%dko = (Ito+IKATP+IKr+IKs+IK1+IKb+Istim-2.0*INaK+ICaK)*Acap/(F*vo) + wash_out*(Ko_Bulk-ko)/settings.tau_wash_out;

% dko = (Ito+IKATP+IKr+IKs+IK1+IKb-2.0*INaK+ICaK)*Acap/(vo*F) + wash_out*(Ko_Bulk-ko)/settings.tau_wash_out; %
dko=0;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%output the state vector when ode_flag==1, and the calculated currents and fluxes otherwise
if flag_ode==1
    output=[dv dnai dnass dki dkss dcai dcass dcansr dcajsr dm dhf dhs dj dhsp djp dmL dhL dhLp da diF diS dap diFp diSp dd dff dfs dfcaf dfcas djca dnca dffp dfcafp dxrf dxrs dxs1 dxs2 dxk1 dJrelnp dJrelp dCaMKt dko]';
else
