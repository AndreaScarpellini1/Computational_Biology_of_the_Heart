function [Ti,StateVars,currents]=main2019_ORd_MMChA(gAmps,settings,isq_act,corr_act,batchYN)
...
beats = round(msSimTotal/CL);  %beats=1080%Lucia +240;
...
X0=[v nai nass ki kss cai cass cansr cajsr m hf hs j hsp jp mL hL hLp a iF iS ap iFp iSp d ff fs fcaf fcas jca nca ffp fcafp xrf xrs xs1 xs2 xk1 Jrelnp Jrelp CaMKt ko]';
...
t0 = clock;
if batchYN==0
    h4 = waitbar(0,'1','Name',' Calculating Action Potential Nº           ','CreateCancelBtn','setappdata(gcbf,''canceling'',1)');
...
for n=1:beats-3
    tspan=[0, CL];
    [time, X]=ode15s(@model2019_ORd_MMChA,tspan,X0,options,gAmps,settings,isq_act,corr_act,msIsqIni,msIsqTotal,n,1);
   ...
   % Update waitbar and message
    ...
    X0=X(size(X,1),:);  %size(X,1) returns the number of rows of X -> X0 = last row of X  
...
settings.stimulation=0; %Not stimulating: last simulation
tspan=[0:0.1:10000];
[time, X]=ode15s(@model2019_ORd_MMChA,tspan,X0,options,gAmps,settings,isq_act,corr_act,msIsqIni,msIsqTotal,n,1);
%hold
%plot(CL+time,X(:,1)); %Plotting v at the end of simulations 

minTi_proto = Ti/(60*1000);   %time in minutes
...
%     IsJs=model2019_ORd_MMChA(Ti(i),StateVars(i,:),settings,isq_act,corr_act,msIsqIni,msIsqTotal,beat_actual,0);
...
function settings = setDefaultSettings(settings)
    if ~isfield(settings, 'minIsqTotal'), settings.minIsqTotal = 0.0; end
    if ~isfield(settings, 'minIsqIni'), settings.minIsqIni = 15; end
    if ~isfield(settings, 'BCL'), settings.BCL = 1000; end
    if ~isfield(settings, 'stimulation'), settings.stimulation = 1; end   
...
function isq_act = setDefaultIsqAct(isq_act)
    if ~isfield(isq_act, 'O2'), isq_act.O2 = 0; end
    if ~isfield(isq_act, 'pH'), isq_act.pH = 0; end
    if ~isfield(isq_act, 'Met'), isq_act.Met = 0; end
    if ~isfield(isq_act, 'wash_out_isq'), isq_act.wash_out_isq = 0; end    
end

function corr_act = setDefaultCorrAct(corr_act)
    if ~isfield(corr_act, 'IKATP'), corr_act.IKATP = 0; end
    if ~isfield(corr_act, 'INaK'), corr_act.INaK = 0; end
    if ~isfield(corr_act, 'SERCA'), corr_act.SERCA = 0; end
    if ~isfield(corr_act, 'IpCa'), corr_act.IpCa = 0; end
    if ~isfield(corr_act, 'INa'), corr_act.INa = 0; end
    if ~isfield(corr_act, 'INaL'), corr_act.INaL = 0; end
    if ~isfield(corr_act, 'ICaL'), corr_act.ICaL = 0; end
    if ~isfield(corr_act, 'NCX'), corr_act.NCX = 0; end
end
